...
(define (impl buf)
  (let* (
         ...
         (choose buf ((choose bvand bvor) (choose buf (bv #xf1 32)) (choose buf (bv #xf1 32) (bv 0 32))) )
         (choose (bv #x000000f1 32) (bv 0 32) buf ))]
...
(define sol
   (synthesize
     #:forall    (list buf)
     #:guarantee (begin
      (assert (and (bveq (impl buf) (spec buf)) (= total-cost 1)))
    ))
)
