...
(define (spec fdhi fdlo bufhi buflo)
  (cond
    [(or (bvne (bvand fdhi bufhi) (bv #xf1f1f1f1 32)) (bvne (bvand fdlo buflo) (bv 0 32))) (bv 0 1)] ;; ALLOW
    [else               (bv 1 1)]))   ;; KILL

(define (impl fdhi fdlo bufhi buflo)
  (let* (
         [cond-expr0 ((choose bveq bvult bvugt bvule bvuge bvne) 
         (choose fdhi fdlo bufhi buflo ((choose bvand bvor) (choose fdhi fdlo bufhi buflo ) (choose fdhi fdlo bufhi buflo (bv #xf1f1f1f1 32) (bv 0 32))) )
         (choose (bv #xf1f1f1f1 32) (bv 0 32) fdhi fdlo bufhi buflo))]
...
(define sol
   (synthesize
     ...
     #:guarantee (begin
      (assert (and (bveq (impl fdhi fdlo bufhi buflo) (spec fdhi fdlo bufhi buflo)) (= total-cost 2)))
    ))
)
