...
(define (impl bufhi buflo)
  (let* (
         ...
         (choose bufhi buflo ((choose bvand bvor) (choose bufhi buflo ) (choose bufhi buflo (?? int32?))) )
         (choose (?? int32?) bufhi buflo ))]
...
(define sol
   (synthesize
     ...
     #:guarantee (begin
      (assert (and (bveq (impl bufhi buflo) (spec bufhi buflo))))
    ))
)
