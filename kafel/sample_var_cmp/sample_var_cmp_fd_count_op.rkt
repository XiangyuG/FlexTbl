...
(define (spec fdhi fdlo counthi countlo)
  (cond
    [(or (bvne fdhi counthi) (bvne fdlo countlo)) (bv 0 1)] ;; ALLOW
    [else               (bv 1 1)]))   ;; KILL

(define (impl fdhi fdlo counthi countlo)
  (let* (
         [cond-expr0 ((choose bveq bvult bvugt bvule bvuge (not bveq)) 
         (choose fdhi fdlo counthi countlo ((choose bvand bvor) (choose fdhi fdlo counthi countlo) (choose fdhi fdlo counthi countlo)))
         (choose (bv 0 32) fdhi fdlo counthi countlo))]
...
(define sol
   (synthesize
     #:forall    (list fdhi fdlo counthi countlo)
     #:guarantee (begin
      (assert (and (bveq (impl fdhi fdlo counthi countlo) (spec fdhi fdlo counthi countlo)) (= total-cost 2)))
    ))
)
